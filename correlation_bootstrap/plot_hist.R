...
plot_histogram=function(filename){
  ...
  # prepare legend
  legendlabels=c("95% CI (Basic)","SEM","Median","Mean")
  names(tableau_colors)=legendlabels
  
  # prepare plots of CIs and centrality
  linesize=0.5
  vlines=data.frame(values=c(CIdata$lower_percentile,CIdata$lower_SEM,CIdata$middle,CIdata$mean),Legend=legendlabels)
  ...
  p=ggplot(histogramdata, aes(x=slope, y=frequency)) + 
    ...
    geom_vline(xintercept = CIdata$upper_percentile,show.legend=FALSE,color=tableau_colors[1], size=linesize) +
